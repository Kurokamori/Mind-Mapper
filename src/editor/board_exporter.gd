...
func _write_pdf(pages: Array, path: String) -> bool:
	...
	var object_count: int = 1 + 1 + pages.size() * 3
	var pages_kids: Array = []
	for i in range(pages.size()):
		pages_kids.append(3 + i * 3)
	...
	for i in range(pages.size()):
		...
		var img: Image = (pages[i] as Dictionary)["image"]
		...
		var raw_rgb: PackedByteArray = img.get_data()
		offsets.append(int(written[0]))
