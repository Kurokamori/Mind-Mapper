class_name MobileMergeResolutionSheet
extends Window
...
@onready var _actions_root: VBoxContainer = %ActionsRoot
...
@onready var _defer_to_desktop_button: Button = %DeferToDesktopButton
...
func _ready() -> void:
	...
	_defer_to_desktop_button.pressed.connect(_on_defer_to_desktop_pressed)
...
func configure(op_kind: String, client_name: String, conflicting_paths: Array, incoming_only_paths: Array, project_name: String) -> void:
	_op_kind = op_kind
	_client_name = client_name if client_name.strip_edges() != "" else "Desktop"
	...
	_defer_to_desktop_button.visible = conflict_count > 0
	_defer_to_desktop_button.tooltip_text = "Send the conflict list to %s so the merge can be resolved on the desktop." % _client_name
	_review_panel.visible = false
...
func _on_defer_to_desktop_pressed() -> void:
	_finalize(LanSyncProtocol.DECISION_DEFER_TO_REQUESTER, {})
