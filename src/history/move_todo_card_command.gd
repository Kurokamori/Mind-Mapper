class_name MoveTodoCardCommand
extends HistoryCommand

var _editor: Node
var _source_list_id: String
var _target_list_id: String
...
func _init(
	...
	_editor = editor
	_source_list_id = source_list_id
	_target_list_id = target_list_id
...
func _apply(src: Array, tgt: Array) -> void:
	if _source_list_id == _target_list_id:
		var only: BoardItem = _editor.find_item_by_id(_source_list_id)
		if only != null:
			only.apply_property("cards", src)
	else:
		var s: BoardItem = _editor.find_item_by_id(_source_list_id)
		if s != null:
			s.apply_property("cards", src)
		var t: BoardItem = _editor.find_item_by_id(_target_list_id)
		if t != null:
			t.apply_property("cards", tgt)
	_emit_op(src, tgt)
...
func _emit_op(src: Array, tgt: Array) -> void:
	...
	OpBus.record_local_change(OpKinds.SET_ITEM_PROPERTY, {"item_id": _source_list_id, "key": "cards", "value": src}, board_id)
	if _source_list_id != _target_list_id:
		OpBus.record_local_change(OpKinds.SET_ITEM_PROPERTY, {"item_id": _target_list_id, "key": "cards", "value": tgt}, board_id)


func description() -> String:
	return "Move todo card"
