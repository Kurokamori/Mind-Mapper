class_name AddStrokesCommand
extends HistoryCommand

var _editor: Node
var _stroke_dicts: Array
...
func _init(editor: Node, stroke_dicts: Array) -> void:
	...
	for entry: Variant in stroke_dicts:
		if typeof(entry) == TYPE_DICTIONARY:
			_stroke_dicts.append(AnnotationStroke.normalize((entry as Dictionary).duplicate(true)))


func do() -> void:
	var board_id: String = AppState.current_board.id if AppState.current_board != null else ""
	for stroke_dict: Dictionary in _stroke_dicts:
...
func primary_op_kind() -> String:
	return OpKinds.CREATE_STROKE


func description() -> String:
	if _stroke_dicts.size() == 1:
		return "Add stroke"
	return "Add %d strokes" % _stroke_dicts.size()
