...
var _item: MapPageNode
var _editor: Node
var _binders: Dictionary = {}
var _suppress_signals: bool = false
var _map_id_by_index: Dictionary = {}


func bind(item: MapPageNode) -> void:
	_item = item
...
func _populate_retarget() -> void:
	...
	_map_id_by_index[idx] = ""
	idx += 1
	var current_idx: int = 0
	for entry_v: Variant in maps:
		var entry: Dictionary = entry_v
		var mid: String = String(entry.get("id", ""))
		var mname: String = String(entry.get("name", ""))
		_retarget_option.add_item(mname, idx)
		_map_id_by_index[idx] = mid
		if mid == _item.target_map_page_id:
			current_idx = idx
		idx += 1
	if current_idx >= 0:
...
func _on_open_pressed() -> void:
	if _item == null:
		return
	var id: String = _item.ensure_target_map_page()
	if id == "":
		return
...
func _on_retarget_selected(index: int) -> void:
	if _suppress_signals or _item == null:
		return
	var id: String = String(_map_id_by_index.get(index, ""))
	if id == _item.target_map_page_id:
		return
	if id == "":
		_item.target_map_page_id = ""
		_item.ensure_target_map_page()
	else:
		var prev: String = _item.target_map_page_id
		var binder: PropertyBinder = PropertyBinder.new(_editor, _item, "target_map_page_id", prev)
		binder.live(id)
		binder.commit(id)
